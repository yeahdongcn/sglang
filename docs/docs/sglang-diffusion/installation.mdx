...
SGLang Diffusion uses PyTorch MPS. The `all_mps` extra also installs the tested
Torch and MLX dependencies required by the SRT MPS backend, but Diffusion does
not use the SRT MLX operators.
