...
AI agents should start with [`AGENTS.md`](AGENTS.md), which is the scoped
instruction entrypoint for this directory. Then use
[`METAL_DOC_INDEX.md`](METAL_DOC_INDEX.md) as the routing table for final
serving results, completion audit, accepted hybrid runtime state, remaining
raw-kernel caveat, and rejected directions that should not be retried without
new evidence. Use [`METAL_TEST_RECORDS_GUIDE.md`](METAL_TEST_RECORDS_GUIDE.md)
before adding new benchmark, correctness, or rejection records.
