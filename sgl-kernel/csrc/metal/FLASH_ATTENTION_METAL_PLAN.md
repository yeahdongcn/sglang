...
- Status: historical proof-of-concept plan.
- Use when: checking early API/wrapper intent or why old per-call flags existed.
- Do not use as: current serving architecture. Open
  `FULL_FLASH_ATTENTION_METAL_PLAN.md` and `METAL_DOC_INDEX.md` instead.
- Search tags: `historical`, `proof-of-concept`, `wrapper-api`,
  `old-env-flags`.
