...
constexpr uint32_t kDecodeThreadsPerGroup = 128;
constexpr uint32_t kOnlineDecodeThreadsPerGroup = 256;
...
std::string dtype_suffix(Dtype dtype, const char* op_name, bool allow_bfloat16 = false) {
  switch (dtype) {
    case float16:
      return "half";
    case float32:
  ...
  auto message = std::string(op_name) + " supports only float16 and float32 arrays";
  if (allow_bfloat16) {
    message = std::string(op_name) + " supports only float16, bfloat16, and float32 arrays";
  }
...
std::string bool_tag(bool value) {
  return value ? "t" : "n";
}
...
struct PagedCacheMeta {
  int32_t num_blocks;
  int32_t block_size;
  int32_t num_kv_heads;
  int32_t head_dim;
  int32_t block_stride;
  int32_t offset_stride;
  int32_t head_stride;
  int32_t dim_stride;
};

bool is_paged_cache_shape(const array& x) {
  return x.ndim() == 4 || x.ndim() == 3;
}

PagedCacheMeta paged_cache_meta(const array& x, const char* name) {
  if (x.ndim() == 4) {
    return PagedCacheMeta{
        checked_dim(x, 0, name),
        checked_dim(x, 1, name),
        checked_dim(x, 2, name),
        checked_dim(x, 3, name),
        checked_stride(x, 0, name),
        checked_stride(x, 1, name),
        checked_stride(x, 2, name),
        checked_stride(x, 3, name),
    };
  }
  if (x.ndim() == 3) {
    return PagedCacheMeta{
  ...
  throw std::runtime_error(
      std::string(name) + " must have shape (num_blocks, block_size, KVH, D) or p1 shape (num_blocks, KVH, D)");
}
...
void validate_decode_paged_inputs(
  ...
  if (!is_paged_cache_shape(k_cache) || !is_paged_cache_shape(v_cache)) {
    throw std::runtime_error(
        "decode_attention_paged K/V caches must have shape (num_blocks, block_size, KVH, D) or p1 shape (num_blocks, "
        "KVH, D)");
  }
  ...
  auto cache = paged_cache_meta(k_cache, "decode_attention_paged key cache");
  if (q.shape(3) != static_cast<size_t>(cache.head_dim)) {
    throw std::runtime_error("decode_attention_paged head dimensions must match");
  }
  if (cache.num_kv_heads == 0 || q.shape(1) % static_cast<size_t>(cache.num_kv_heads) != 0) {
  ...
  if (q.shape(0) > 0 && cache.num_blocks == 0) {
  ...
  auto max_seq_len = block_tables.shape(1) * static_cast<size_t>(cache.block_size);
...
void dispatch_decode_attention_paged(
  ...
  auto cache_block_stride = cache.block_stride;
  auto cache_offset_stride = cache.offset_stride;
  auto cache_head_stride = cache.head_stride;
  auto cache_dim_stride = cache.dim_stride;
  ...
  auto& device = metal::device(Device::gpu);
  auto* library = device.get_library(kMetalLibraryName);
  auto dtype = dtype_suffix(q.dtype(), "decode_attention_paged");
  auto use_online_kernel = head_dim == 128 && block_size == 16;
  auto use_vector_online_kernel =
      use_online_kernel && q.dtype() == float16 && q_dim_stride == 1 && cache_dim_stride == 1 && out_dim_stride == 1;
  auto kernel_name = use_vector_online_kernel
                         ? std::string("sgl_metal_decode_attention_paged_h128_b16_online_vec_half")
                         : std::string(
                               use_online_kernel ? "sgl_metal_decode_attention_paged_h128_b16_online_"
                                                 : "sgl_metal_decode_attention_paged_") +
                               dtype;
  auto* kernel = device.get_kernel(kernel_name, library, kernel_name);
  ...
  encoder.dispatch_threadgroups(
      MTL::Size::Make(num_heads, batch, 1),
      MTL::Size::Make(use_online_kernel ? kOnlineDecodeThreadsPerGroup : kDecodeThreadsPerGroup, 1, 1));
...
void dispatch_decode_attention_paged_with_kv(
    array& out,
    const array& q,
    const array& k_new,
    const array& v_new,
  ...
  if (q.dtype() != float16 || q.shape(3) != 128 || k_cache.shape(1) != 16) {
    throw std::runtime_error("decode_attention_paged_with_kv supports only float16 head_dim=128 block_size=16");
  }
  if (k_new.ndim() != 3 || v_new.ndim() != 3 || k_new.shape() != v_new.shape()) {
    throw std::runtime_error("decode_attention_paged_with_kv K/V tensors must have shape (B, KVH, D)");
  }
  if (k_new.shape(0) != q.shape(0) || k_new.shape(1) != k_cache.shape(2) || k_new.shape(2) != q.shape(3)) {
    throw std::runtime_error("decode_attention_paged_with_kv K/V tensor shapes must match query and cache");
  }
  if (k_new.dtype() != q.dtype() || v_new.dtype() != q.dtype()) {
    throw std::runtime_error("decode_attention_paged_with_kv query and K/V tensor dtypes must match");
  }
  if (slot_mapping.ndim() != 1 || slot_mapping.shape(0) != q.shape(0)) {
    throw std::runtime_error("decode_attention_paged_with_kv slot_mapping must have shape (B,)");
  }
  if (slot_mapping.dtype() != int32) {
    throw std::runtime_error("decode_attention_paged_with_kv slot_mapping must be int32");
  }
  validate_dense_row_contiguous(k_new, "decode_attention_paged_with_kv key tensor");
  validate_dense_row_contiguous(v_new, "decode_attention_paged_with_kv value tensor");
  validate_dense_row_contiguous(slot_mapping, "decode_attention_paged_with_kv slot_mapping");

  if (q.size() == 0) {
    return;
  }
  require_registered_library("decode_attention_paged_with_kv");

  auto batch = checked_dim(q, 0, "decode_attention_paged_with_kv query");
  auto num_heads = checked_dim(q, 1, "decode_attention_paged_with_kv query");
  auto num_kv_heads = checked_dim(k_cache, 2, "decode_attention_paged_with_kv key cache");
  auto max_blocks = checked_dim(block_tables, 1, "decode_attention_paged_with_kv block_tables");
  auto q_batch_stride = checked_stride(q, 0, "decode_attention_paged_with_kv query");
  auto q_head_stride = checked_stride(q, 1, "decode_attention_paged_with_kv query");
  auto q_dim_stride = checked_stride(q, 3, "decode_attention_paged_with_kv query");
  auto new_token_stride = checked_stride(k_new, 0, "decode_attention_paged_with_kv key tensor");
  auto new_head_stride = checked_stride(k_new, 1, "decode_attention_paged_with_kv key tensor");
  auto new_dim_stride = checked_stride(k_new, 2, "decode_attention_paged_with_kv key tensor");
  auto value_new_token_stride = checked_stride(v_new, 0, "decode_attention_paged_with_kv value tensor");
  auto value_new_head_stride = checked_stride(v_new, 1, "decode_attention_paged_with_kv value tensor");
  auto value_new_dim_stride = checked_stride(v_new, 2, "decode_attention_paged_with_kv value tensor");
  auto cache_block_stride = checked_stride(k_cache, 0, "decode_attention_paged_with_kv key cache");
  auto cache_offset_stride = checked_stride(k_cache, 1, "decode_attention_paged_with_kv key cache");
  auto cache_head_stride = checked_stride(k_cache, 2, "decode_attention_paged_with_kv key cache");
  auto cache_dim_stride = checked_stride(k_cache, 3, "decode_attention_paged_with_kv key cache");
  auto value_cache_block_stride = checked_stride(v_cache, 0, "decode_attention_paged_with_kv value cache");
  auto value_cache_offset_stride = checked_stride(v_cache, 1, "decode_attention_paged_with_kv value cache");
  auto value_cache_head_stride = checked_stride(v_cache, 2, "decode_attention_paged_with_kv value cache");
  auto value_cache_dim_stride = checked_stride(v_cache, 3, "decode_attention_paged_with_kv value cache");
  auto out_batch_stride = checked_stride(out, 0, "decode_attention_paged_with_kv output");
  auto out_head_stride = checked_stride(out, 1, "decode_attention_paged_with_kv output");
  auto out_dim_stride = checked_stride(out, 3, "decode_attention_paged_with_kv output");
  auto block_tables_batch_stride = checked_stride(block_tables, 0, "decode_attention_paged_with_kv block_tables");
  auto block_tables_block_stride = checked_stride(block_tables, 1, "decode_attention_paged_with_kv block_tables");
  auto context_lens_stride = checked_stride(context_lens, 0, "decode_attention_paged_with_kv context_lens");
  auto slot_mapping_stride = checked_stride(slot_mapping, 0, "decode_attention_paged_with_kv slot_mapping");
  if (q_dim_stride != 1 || new_dim_stride != 1 || value_new_dim_stride != 1 || cache_dim_stride != 1 ||
      value_cache_dim_stride != 1 || out_dim_stride != 1 || new_token_stride != value_new_token_stride ||
      new_head_stride != value_new_head_stride || cache_block_stride != value_cache_block_stride ||
      cache_offset_stride != value_cache_offset_stride || cache_head_stride != value_cache_head_stride) {
    throw std::runtime_error("decode_attention_paged_with_kv requires row-contiguous matching K/V strides");
  }

  auto stream = default_stream(Device::gpu);
  auto& device = metal::device(Device::gpu);
  auto* library = device.get_library(kMetalLibraryName);
  auto kernel_name = std::string("sgl_metal_decode_attention_paged_h128_b16_fused_kv_vec_half");
  auto* kernel = device.get_kernel(kernel_name, library, kernel_name);

  auto& encoder = device.get_command_encoder(stream.index);
  encoder.set_compute_pipeline_state(kernel);
  encoder.set_input_array(q, 0);
  encoder.set_input_array(k_new, 1);
  encoder.set_input_array(v_new, 2);
  encoder.set_output_array(k_cache, 3);
  encoder.set_output_array(v_cache, 4);
  encoder.set_output_array(out, 5);
  encoder.set_input_array(block_tables, 6);
  encoder.set_input_array(context_lens, 7);
  encoder.set_input_array(slot_mapping, 8);
  encoder.set_bytes(scale, 9);
  encoder.set_bytes(batch, 10);
  encoder.set_bytes(num_heads, 11);
  encoder.set_bytes(num_kv_heads, 12);
  encoder.set_bytes(max_blocks, 13);
  encoder.set_bytes(q_batch_stride, 14);
  encoder.set_bytes(q_head_stride, 15);
  encoder.set_bytes(new_token_stride, 16);
  encoder.set_bytes(new_head_stride, 17);
  encoder.set_bytes(cache_block_stride, 18);
  encoder.set_bytes(cache_offset_stride, 19);
  encoder.set_bytes(cache_head_stride, 20);
  encoder.set_bytes(out_batch_stride, 21);
  encoder.set_bytes(out_head_stride, 22);
  encoder.set_bytes(block_tables_batch_stride, 23);
  encoder.set_bytes(block_tables_block_stride, 24);
  encoder.set_bytes(context_lens_stride, 25);
  encoder.set_bytes(slot_mapping_stride, 26);

  encoder.dispatch_threadgroups(
      MTL::Size::Make(num_heads, batch, 1), MTL::Size::Make(kOnlineDecodeThreadsPerGroup, 1, 1));

  device.add_temporary(q, stream.index);
  device.add_temporary(k_new, stream.index);
  device.add_temporary(v_new, stream.index);
  device.add_temporary(k_cache, stream.index);
  device.add_temporary(v_cache, stream.index);
  device.add_temporary(out, stream.index);
  device.add_temporary(block_tables, stream.index);
...
bool get_uniform_varlen_lens(
    ...
    int32_t& q_len,
    int32_t& k_len) {
  if (num_seqs <= 0 || total_q <= 0 || total_k <= 0) {
    return false;
  }

  auto cu_q_host = cu_seqlens_q;
  auto cu_k_host = cu_seqlens_k;
  cu_q_host.eval();
  cu_k_host.eval();
  const auto* cu_q = cu_q_host.data<int32_t>();
  const auto* cu_k = cu_k_host.data<int32_t>();
  if (cu_q[0] != 0 || cu_k[0] != 0 || cu_q[num_seqs * cu_q_stride] != total_q ||
      cu_k[num_seqs * cu_k_stride] != total_k) {
    return false;
  }

  const int32_t first_q_len = cu_q[cu_q_stride] - cu_q[0];
  const int32_t first_k_len = cu_k[cu_k_stride] - cu_k[0];
  if (first_q_len <= 0 || first_k_len <= 0) {
    return false;
  }
  for (int32_t seq = 0; seq < num_seqs; ++seq) {
    const int32_t q_start = cu_q[seq * cu_q_stride];
    const int32_t q_end = cu_q[(seq + 1) * cu_q_stride];
    const int32_t k_start = cu_k[seq * cu_k_stride];
    const int32_t k_end = cu_k[(seq + 1) * cu_k_stride];
    if (q_end < q_start || k_end < k_start || q_end - q_start != first_q_len || k_end - k_start != first_k_len) {
      return false;
    }
  }
  q_len = first_q_len;
...
bool prefix_lens_are_zero(const array& prefix_lens, int32_t batch, int32_t prefix_lens_stride) {
  if (batch <= 0) {
    return false;
  }
  ...
  for (int32_t seq = 0; seq < batch; ++seq) {
    if (prefixes[seq * prefix_lens_stride] != 0) {
      return false;
    }
  }
  return true;
...
void dispatch_flash_attn_varlen_steel_h128(
    array& out,
    const array& q,
    const array& k,
    const array& v,
    ...
    int32_t out_token_stride,
    int32_t out_head_stride) {
  constexpr int32_t kBQ = 32;
  constexpr int32_t kBK = 16;
  constexpr int32_t kBD = 128;
  constexpr int32_t kWM = 4;
  constexpr int32_t kWN = 1;
  ...
  const bool align_q = (q_len % kBQ) == 0;
  const bool align_k = (k_len % kBK) == 0;
  const bool has_mask = false;
  const bool do_causal = causal;
  ...
  const std::string base_name = "sgl_steel_attention_float16_bq32_bk16_bd128_wm4_wn1_maskfloat16";
  const std::string hash_name = base_name + "_align_Q_" + bool_tag(align_q) + "_align_K_" + bool_tag(align_k) +
                                "_has_mask_n_do_causal_" + bool_tag(do_causal) + "_has_sinks_n";
  auto* kernel = device.get_kernel(base_name, library, hash_name, func_consts);
  ...
  SteelAttentionParams params{
      ...
      k_len - (k_len / kBK) * kBK,
      k_len - q_len,
      {static_cast<int64_t>(q_len) * q_token_stride, q_head_stride, q_token_stride},
      {static_cast<int64_t>(k_len) * kv_token_stride, kv_head_stride, kv_token_stride},
      {static_cast<int64_t>(k_len) * kv_token_stride, kv_head_stride, kv_token_stride},
      {static_cast<int64_t>(q_len) * out_token_stride, out_head_stride, out_token_stride},
  };
  ...
  encoder.dispatch_threadgroups(MTL::Size::Make(params.NQ, num_heads, batch), MTL::Size::Make(32, kWM, kWN));
...
void dispatch_flash_attn_varlen(
  ...
  int32_t uniform_k_len = 0;
  const bool use_steel_kernel = q.dtype() == float16 && head_dim == 128 && q_dim_stride == 1 && kv_dim_stride == 1 &&
                                out_dim_stride == 1 &&
                                get_uniform_varlen_lens(
                                    cu_seqlens_q,
                                    cu_seqlens_k,
                                    total_q,
                                    checked_dim(k, 0, "flash_attn_varlen key"),
                                    num_seqs,
                                    cu_q_stride,
                                    cu_k_stride,
                                    uniform_q_len,
                                    uniform_k_len);
  if (use_steel_kernel) {
    dispatch_flash_attn_varlen_steel_h128(
        out,
        q,
        k,
        v,
  ...
  auto& device = metal::device(Device::gpu);
  auto* library = device.get_library(kMetalLibraryName);
  auto dtype = dtype_suffix(q.dtype(), "flash_attn_varlen");
  auto use_online_kernel =
      q.dtype() == float16 && head_dim == 128 && q_dim_stride == 1 && kv_dim_stride == 1 && out_dim_stride == 1;
  auto kernel_name = use_online_kernel ? std::string("sgl_metal_flash_attn_varlen_h128_online_vec_half")
                                       : std::string("sgl_metal_flash_attn_varlen_") + dtype;
  auto* kernel = device.get_kernel(kernel_name, library, kernel_name);
  ...
  encoder.dispatch_threadgroups(
      MTL::Size::Make(num_heads, total_q, 1),
      MTL::Size::Make(use_online_kernel ? kOnlineDecodeThreadsPerGroup : kDecodeThreadsPerGroup, 1, 1));
...
void validate_prefill_attention_paged_inputs(
  ...
  if (!is_paged_cache_shape(k_cache) || !is_paged_cache_shape(v_cache)) {
    throw std::runtime_error(
        "prefill_attention_paged K/V caches must have shape (num_blocks, block_size, KVH, D) or p1 shape (num_blocks, "
        "KVH, D)");
  }
  if (out.ndim() != 3 || out.shape() != q.shape()) {
  ...
  auto cache = paged_cache_meta(k_cache, "prefill_attention_paged key cache");
  if (q.shape(2) != k.shape(2) || q.shape(2) != static_cast<size_t>(cache.head_dim)) {
    throw std::runtime_error("prefill_attention_paged head dimensions must match");
  }
  if (k.shape(1) != static_cast<size_t>(cache.num_kv_heads)) {
  ...
  auto num_cache_blocks = static_cast<size_t>(cache.num_blocks);
  auto block_size = static_cast<size_t>(cache.block_size);
...
void dispatch_prefill_attention_paged(
  ...
  int32_t uniform_q_len = 0;
  int32_t uniform_k_len = 0;
  const bool use_steel_no_prefix_kernel =
      q.dtype() == float16 && head_dim == 128 && q_dim_stride == 1 && kv_dim_stride == 1 && out_dim_stride == 1 &&
      prefix_lens_are_zero(prefix_lens, batch, prefix_lens_stride) &&
      get_uniform_varlen_lens(
          cu_seqlens_q, cu_seqlens_q, total_q, total_q, batch, cu_q_stride, cu_q_stride, uniform_q_len, uniform_k_len);
  if (use_steel_no_prefix_kernel) {
    dispatch_flash_attn_varlen_steel_h128(
        out,
        q,
        k,
  ...
  auto& device = metal::device(Device::gpu);
  auto* library = device.get_library(kMetalLibraryName);
  auto dtype = dtype_suffix(q.dtype(), "prefill_attention_paged");
  auto use_online_kernel = q.dtype() == float16 && head_dim == 128 && block_size == 16 && q_dim_stride == 1 &&
                           kv_dim_stride == 1 && cache_dim_stride == 1 && out_dim_stride == 1;
  auto kernel_name = use_online_kernel ? std::string("sgl_metal_prefill_attention_paged_h128_b16_online_vec_half")
                                       : std::string("sgl_metal_prefill_attention_paged_") + dtype;
  auto* kernel = device.get_kernel(kernel_name, library, kernel_name);
...
void validate_paged_kv_scatter_inputs(
  ...
  if (!is_paged_cache_shape(k_cache) || !is_paged_cache_shape(v_cache)) {
    throw std::runtime_error(
        "paged_kv_scatter K/V caches must have shape (num_blocks, block_size, KVH, D) or p1 shape (num_blocks, KVH, "
        "D)");
  }
  if (k.shape() != v.shape()) {
  ...
  dtype_suffix(k.dtype(), "paged_kv_scatter", true);
  if (slot_mapping.dtype() != int32) {
    throw std::runtime_error("paged_kv_scatter slot_mapping must be an int32 array");
  }
  auto cache = paged_cache_meta(k_cache, "paged_kv_scatter key cache");
  if (k.shape(1) != static_cast<size_t>(cache.num_kv_heads)) {
    throw std::runtime_error("paged_kv_scatter KV head counts must match");
  }
  if (k.shape(2) != static_cast<size_t>(cache.head_dim)) {
  ...
  if (k.shape(0) > 0 && cache.num_blocks == 0) {
    throw std::runtime_error("paged_kv_scatter KV cache must contain at least one block");
  }
  auto cache_slot_count = static_cast<size_t>(cache.num_blocks) * static_cast<size_t>(cache.block_size);
...
void dispatch_paged_kv_scatter(
  ...
  auto cache = paged_cache_meta(k_cache, "paged_kv_scatter key cache");
  auto block_size = cache.block_size;
  auto cache_slot_count_size = static_cast<size_t>(cache.num_blocks) * static_cast<size_t>(cache.block_size);
  ...
  auto& device = metal::device(Device::gpu);
  auto* library = device.get_library(kMetalLibraryName);
  auto kernel_name = "sgl_metal_paged_kv_scatter_" + dtype_suffix(k.dtype(), "paged_kv_scatter", true);
  auto* kernel = device.get_kernel(kernel_name, library, kernel_name);
...
void decode_attention_paged_with_kv(
    nb::handle out_h,
    nb::handle q_h,
    nb::handle k_h,
    nb::handle v_h,
  ...
  dispatch_decode_attention_paged_with_kv(
      out, q, k, v, k_cache, v_cache, block_tables, context_lens, slot_mapping, scale);
}
...
NB_MODULE(_metal, m) {
  ...
  m.def(
      ...
      nb::arg("slot_mapping"),
      nb::arg("scale"),
      "Run fused paged decode attention while writing the current token K/V.");
  m.def(
      "flash_attn_varlen",
      &flash_attn_varlen,
      nb::arg("out"),
