...
def _scaled_arange(mx, shape, offset, divisor):
    ...
    out = mx.contiguous((values.reshape(*shape) + offset) / divisor)
    mx.eval(out)
    return out


def _contiguous_cast(mx, x, dtype):
    out = mx.contiguous(x.astype(dtype))
    mx.eval(out)
    return out
...
def _paged_kv_scatter_reference(k, v, k_cache, v_cache, slot_mapping):
    ...
    for token, slot in enumerate(slot_mapping.tolist()):
        if k_cache.ndim == 3:
            expected_k[slot : slot + 1, :, :] = k[token : token + 1]
            expected_v[slot : slot + 1, :, :] = v[token : token + 1]
        else:
...
@pytest.mark.parametrize("dtype,atol", [("float32", 1e-5), ("bfloat16", 1e-2)])
def test_metal_paged_kv_scatter_matches_reference_across_block_boundaries(dtype, atol):
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import paged_kv_scatter

    mlx_dtype = getattr(mx, dtype)
    k = _scaled_arange(mx, (5, 2, 4), -3.0, 11.0).astype(mlx_dtype)
    v = _scaled_arange(mx, (5, 2, 4), 7.0, 13.0).astype(mlx_dtype)
    k_cache = _scaled_arange(mx, (4, 2, 2, 4), 2.0, 17.0).astype(mlx_dtype)
    v_cache = _scaled_arange(mx, (4, 2, 2, 4), -5.0, 19.0).astype(mlx_dtype)
    ...
    assert mx.allclose(
        k_cache.astype(mx.float32), expected_k.astype(mx.float32), atol=atol, rtol=atol
    )
    assert mx.allclose(
        v_cache.astype(mx.float32), expected_v.astype(mx.float32), atol=atol, rtol=atol
    )


def test_metal_paged_kv_scatter_matches_reference_for_p1_flat_cache():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import paged_kv_scatter

    k = _scaled_arange(mx, (3, 2, 4), -3.0, 11.0)
    v = _scaled_arange(mx, (3, 2, 4), 7.0, 13.0)
    k_cache = _scaled_arange(mx, (5, 2, 4), 2.0, 17.0)
    v_cache = _scaled_arange(mx, (5, 2, 4), -5.0, 19.0)
    slot_mapping = mx.array([1, 2, 4], dtype=mx.int32)
    expected_k, expected_v = _paged_kv_scatter_reference(
        k, v, k_cache, v_cache, slot_mapping
    )
...
def test_metal_paged_kv_scatter_all_layers_unchecked_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import paged_kv_scatter_all_layers_unchecked

    layers = 3
    slot_mapping = mx.array([1, 4, 5], dtype=mx.int32)
    k_layers = [
        _scaled_arange(mx, (3, 2, 4), float(layer), 7.0 + layer)
        for layer in range(layers)
    ]
    v_layers = [layer + 10 for layer in k_layers]
    k_caches = [
        _scaled_arange(mx, (4, 2, 2, 4), 2.0 + layer, 17.0) for layer in range(layers)
    ]
    v_caches = [mx.contiguous(cache - 5) for cache in k_caches]
    expected = [
        _paged_kv_scatter_reference(
            k_layers[layer],
            v_layers[layer],
            k_caches[layer],
            v_caches[layer],
    ...
    result = paged_kv_scatter_all_layers_unchecked(
        k_layers, v_layers, k_caches, v_caches, slot_mapping, eager=True
    )
    mx.eval(
        *k_caches,
        *v_caches,
    ...
    assert result is None
    for layer in range(layers):
        expected_k, expected_v = expected[layer]
        assert mx.allclose(k_caches[layer], expected_k, atol=1e-5, rtol=1e-5)
        assert mx.allclose(v_caches[layer], expected_v, atol=1e-5, rtol=1e-5)


@pytest.mark.parametrize("cache_dtype,atol", [("float16", 1e-3), ("bfloat16", 1e-2)])
def test_metal_paged_kv_scatter_all_layers_unchecked_casts_to_cache_dtype(
    cache_dtype, atol
):
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import paged_kv_scatter_all_layers_unchecked

    mlx_cache_dtype = getattr(mx, cache_dtype)
    slot_mapping = mx.array([0, 3], dtype=mx.int32)
    k_layers = [
        mx.array([[[1.25]], [[2.5]]], dtype=mx.float32),
        mx.array([[[3.75]], [[4.5]]], dtype=mx.float32),
    ]
    v_layers = [layer + 20.0 for layer in k_layers]
    k_caches = [
        mx.zeros((2, 2, 1, 1), dtype=mlx_cache_dtype),
        mx.zeros((2, 2, 1, 1), dtype=mlx_cache_dtype),
    ]
    v_caches = [
        mx.zeros((2, 2, 1, 1), dtype=mlx_cache_dtype),
    ...
    expected = [
        _paged_kv_scatter_reference(
            k_layers[layer].astype(mlx_cache_dtype),
            v_layers[layer].astype(mlx_cache_dtype),
            k_caches[layer],
            v_caches[layer],
            slot_mapping,
        )
    ...
    for layer in range(2):
        expected_k, expected_v = expected[layer]
        assert k_caches[layer].dtype == mlx_cache_dtype
        assert v_caches[layer].dtype == mlx_cache_dtype
        assert mx.allclose(
            k_caches[layer].astype(mx.float32),
            expected_k.astype(mx.float32),
            atol=atol,
            rtol=atol,
        )
        assert mx.allclose(
            v_caches[layer].astype(mx.float32),
            expected_v.astype(mx.float32),
            atol=atol,
            rtol=atol,
        )
...
def test_metal_decode_attention_paged_online_h128_b16_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_paged

    q = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 4, 1, 128), -7.0, 4096.0), mx.float16
    )
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (7, 16, 2, 128), -11.0, 4096.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (7, 16, 2, 128), 5.0, 2048.0), mx.float16
    )
    block_tables = mx.array([[1, 2, 3], [4, 5, 6]], dtype=mx.int32)
    context_lens = mx.array([33, 17], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])

    out = decode_attention_paged(q, k_cache, v_cache, block_tables, context_lens, scale)
    ref = _decode_paged_reference(
        q, k_cache, v_cache, [[1, 2, 3], [4, 5, 6]], [33, 17], scale
    )

    mx.eval(out, ref)
    assert out.shape == q.shape
    assert out.dtype == q.dtype
    assert mx.allclose(out, ref, atol=5e-3, rtol=5e-3)


def test_metal_decode_attention_paged_lazy_h128_b16_handles_tiny_block_table():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_paged_lazy_unchecked

    q = _contiguous_cast(
        mx, _scaled_arange(mx, (1, 2, 1, 128), -7.0, 1024.0), mx.float16
    )
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 16, 1, 128), -11.0, 2048.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 16, 1, 128), 5.0, 2048.0), mx.float16
    )
    block_tables = mx.array([[1]], dtype=mx.int32)
    context_lens = mx.array([16], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])

    out = decode_attention_paged_lazy_unchecked(
        q, k_cache, v_cache, block_tables, context_lens, scale
    )
    ref = _decode_paged_reference(q, k_cache, v_cache, [[1]], [16], scale)
...
def test_metal_decode_attention_paged_p1_lazy_h128_matches_reference():
    mx = pytest.importorskip("mlx.core")
    ...
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (9, 1, 2, 128), -11.0, 4096.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (9, 1, 2, 128), 5.0, 2048.0), mx.float16
    )
    block_tables = mx.array([[2, 0, 4, 7, 8], [1, 3, 5, 6, 0]], dtype=mx.int32)
    context_lens = mx.array([5, 4], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])

    out = decode_attention_paged_p1_lazy_unchecked(
        q, k_cache, v_cache, block_tables, context_lens, scale
    )
    ref = _decode_paged_reference(
        q, k_cache, v_cache, [[2, 0, 4, 7, 8], [1, 3, 5, 6, 0]], [5, 4], scale
    )
...
def test_metal_decode_attention_paged_p1_lazy_bf16_h128_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_paged_p1_lazy_unchecked

    q = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 4, 1, 128), -7.0, 4096.0), mx.bfloat16
    )
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (9, 1, 2, 128), -11.0, 4096.0), mx.bfloat16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (9, 1, 2, 128), 5.0, 2048.0), mx.bfloat16
    )
    block_tables = mx.array([[2, 0, 4, 7, 8], [1, 3, 5, 6, 0]], dtype=mx.int32)
    context_lens = mx.array([5, 4], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
    ...
    assert out.shape == q.shape
    assert out.dtype == q.dtype
    assert mx.allclose(out, ref, atol=2e-2, rtol=2e-2)


def test_metal_decode_attention_dense_h128_lazy_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_dense_h128_unchecked

    q = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 4, 1, 128), -7.0, 4096.0), mx.float16
    )
    k = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 2, 19, 128), -11.0, 4096.0), mx.float16
    )
    v = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 2, 19, 128), 5.0, 2048.0), mx.float16
    )
    scale = 1 / math.sqrt(q.shape[-1])
    ref = _decode_reference(
        q.astype(mx.float32), k.astype(mx.float32), v.astype(mx.float32), scale
    ...
    for mode, threads in (
        ...
        out = decode_attention_dense_h128_unchecked(
            q,
            k,
            v,
            scale,
        ...
        assert out.shape == q.shape
        assert out.dtype == q.dtype
        assert mx.allclose(out.astype(mx.float32), ref, atol=5e-3, rtol=5e-3)


def test_metal_decode_attention_dense_gqa2_h128_lazy_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_dense_gqa2_h128_unchecked
    ...
    for threads in (128, 256):
        out = decode_attention_dense_gqa2_h128_unchecked(
            q,
            k,
            v,
            scale,
...
def test_metal_decode_attention_paged_with_kv_h128_b16_matches_reference_and_writes_cache():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import decode_attention_paged_with_kv

    q = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 4, 1, 128), -7.0, 4096.0), mx.float16
    )
    k_new = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 2, 128), -31.0, 3072.0), mx.float16
    )
    v_new = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 2, 128), 19.0, 1536.0), mx.float16
    )
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (7, 16, 2, 128), -11.0, 4096.0), mx.float16
    )
    ...
    block_tables = mx.array([[3], [5]], dtype=mx.int32)
    context_lens = mx.array([1, 1], dtype=mx.int32)
    slot_mapping = mx.array([3 * 16, 5 * 16], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])

    out = decode_attention_paged_with_kv(
        q,
        k_new,
        v_new,
        k_cache,
    ...
    ref = mx.fast.scaled_dot_product_attention(
        q, k_new[:, :, None, :], v_new[:, :, None, :], scale=scale
    )

    mx.eval(out, ref, k_cache, v_cache)
    assert out.shape == q.shape
    assert out.dtype == q.dtype
    assert mx.allclose(out, ref, atol=5e-3, rtol=5e-3)
    assert mx.allclose(k_cache[3, 0], k_new[0], atol=0, rtol=0)
    assert mx.allclose(v_cache[3, 0], v_new[0], atol=0, rtol=0)
    assert mx.allclose(k_cache[5, 0], k_new[1], atol=0, rtol=0)
    assert mx.allclose(v_cache[5, 0], v_new[1], atol=0, rtol=0)
...
def test_metal_flash_attn_varlen_func_h128_online_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import flash_attn_varlen_func

    q = _contiguous_cast(mx, _scaled_arange(mx, (5, 4, 128), -3.0, 4096.0), mx.float16)
    k = _contiguous_cast(mx, _scaled_arange(mx, (9, 2, 128), 2.0, 4096.0), mx.float16)
    v = _contiguous_cast(mx, _scaled_arange(mx, (9, 2, 128), -5.0, 2048.0), mx.float16)
    cu_q = mx.array([0, 2, 5], dtype=mx.int32)
    cu_k = mx.array([0, 4, 9], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
    ...
    ref = _flash_attn_varlen_reference(
        q, k, v, [0, 2, 5], [0, 4, 9], scale, causal=True
    )
...
def test_metal_flash_attn_varlen_func_h128_uniform_prefix_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import flash_attn_varlen_func
    ...
    k_len = 48
    q = _contiguous_cast(
        mx, _scaled_arange(mx, (q_len * 2, 4, 128), -3.0, 16384.0), mx.float16
    )
    k = _contiguous_cast(
        mx, _scaled_arange(mx, (k_len * 2, 2, 128), 2.0, 16384.0), mx.float16
    )
    v = _contiguous_cast(
        mx, _scaled_arange(mx, (k_len * 2, 2, 128), -5.0, 8192.0), mx.float16
    )
    cu_q = mx.array([0, q_len, q_len * 2], dtype=mx.int32)
    cu_k = mx.array([0, k_len, k_len * 2], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
    ...
    ref = _flash_attn_varlen_reference(
        q, k, v, [0, q_len, q_len * 2], [0, k_len, k_len * 2], scale, causal=True
    )
...
def test_metal_prefill_attention_paged_float16_partial_prefix_matches_reference():
    ...
    q = _contiguous_cast(mx, _scaled_arange(mx, (5, 4, 4), -3.0, 11.0), mx.float16)
    k = _contiguous_cast(mx, _scaled_arange(mx, (5, 2, 4), 2.0, 13.0), mx.float16)
    v = _contiguous_cast(mx, _scaled_arange(mx, (5, 2, 4), -5.0, 17.0), mx.float16)
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (6, 2, 2, 4), -11.0, 19.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (6, 2, 2, 4), 7.0, 23.0), mx.float16
    )
    block_tables = mx.array([[2, 0, 4], [1, 3, 5]], dtype=mx.int32)
    prefix_lens = mx.array([3, 5], dtype=mx.int32)
    cu_q = mx.array([0, 2, 5], dtype=mx.int32)
...
def test_metal_prefill_attention_paged_h128_online_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import prefill_attention_paged

    q = _contiguous_cast(mx, _scaled_arange(mx, (5, 4, 128), -3.0, 16384.0), mx.float16)
    k = _contiguous_cast(mx, _scaled_arange(mx, (5, 2, 128), 2.0, 16384.0), mx.float16)
    v = _contiguous_cast(mx, _scaled_arange(mx, (5, 2, 128), -5.0, 8192.0), mx.float16)
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (6, 16, 2, 128), -11.0, 16384.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (6, 16, 2, 128), 7.0, 8192.0), mx.float16
    )
    block_tables = mx.array([[1, 2], [3, 4]], dtype=mx.int32)
    prefix_lens = mx.array([17, 31], dtype=mx.int32)
    cu_q = mx.array([0, 2, 5], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
...
def test_metal_prefill_attention_paged_h128_no_prefix_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import prefill_attention_paged
    ...
    total_q = seq_len * 2
    q = _contiguous_cast(
        mx, _scaled_arange(mx, (total_q, 4, 128), -3.0, 16384.0), mx.float16
    )
    k = _contiguous_cast(
        mx, _scaled_arange(mx, (total_q, 2, 128), 2.0, 16384.0), mx.float16
    )
    v = _contiguous_cast(
        mx, _scaled_arange(mx, (total_q, 2, 128), -5.0, 8192.0), mx.float16
    )
    k_cache = mx.zeros((1, 16, 2, 128), dtype=mx.float16)
    v_cache = mx.zeros((1, 16, 2, 128), dtype=mx.float16)
    block_tables = mx.array([[0], [0]], dtype=mx.int32)
    prefix_lens = mx.array([0, 0], dtype=mx.int32)
    cu_q = mx.array([0, seq_len, total_q], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
...
def test_metal_prefill_attention_paged_h128_uniform_prefix_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import prefill_attention_paged
    ...
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 16, 2, 128), -11.0, 16384.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (2, 16, 2, 128), 7.0, 8192.0), mx.float16
    )
    block_tables = mx.array([[0], [1]], dtype=mx.int32)
    prefix_lens = mx.array([prefix_len, prefix_len], dtype=mx.int32)
    cu_q = mx.array([0, seq_len, total_q], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
...
def test_metal_prefill_attention_paged_h128_p1_uniform_prefix_matches_reference():
    mx = pytest.importorskip("mlx.core")
    from sgl_kernel.metal import prefill_attention_paged
    ...
    prefix_len = 33
    q = _contiguous_cast(
        mx, _scaled_arange(mx, (seq_len, 4, 128), -3.0, 16384.0), mx.float16
    )
    k = _contiguous_cast(
        mx, _scaled_arange(mx, (seq_len, 2, 128), 2.0, 16384.0), mx.float16
    )
    v = _contiguous_cast(
        mx, _scaled_arange(mx, (seq_len, 2, 128), -5.0, 8192.0), mx.float16
    )
    k_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (prefix_len, 1, 2, 128), -11.0, 16384.0), mx.float16
    )
    v_cache = _contiguous_cast(
        mx, _scaled_arange(mx, (prefix_len, 1, 2, 128), 7.0, 8192.0), mx.float16
    )
    block_tables = mx.array([list(range(prefix_len))], dtype=mx.int32)
    prefix_lens = mx.array([prefix_len], dtype=mx.int32)
    cu_q = mx.array([0, seq_len], dtype=mx.int32)
    scale = 1 / math.sqrt(q.shape[-1])
