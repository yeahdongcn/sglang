...
def test_top_k_top_p_sampling_from_probs_logits_top_k_first_alignment(
    ...
    logits = torch.randn(batch_size, vocab_size, device=get_device(0)) * 5
    generator_logits = torch.Generator(get_device(0))
...
def test_top_k_top_p_joint_sampling_from_probs(batch_size, vocab_size, p):
    ...
    eps = 1e-4
    pre_norm_prob = torch.rand(batch_size, vocab_size, device=get_device(0))
    normalized_prob = pre_norm_prob / pre_norm_prob.sum(dim=-1, keepdim=True)
    # top-p mask
    sorted_prob, indices = torch.sort(normalized_prob, descending=False)
    cdf = torch.cumsum(sorted_prob, dim=-1)
    mask_top_p = torch.zeros(
        batch_size, vocab_size, dtype=torch.int32, device=get_device(0)
    )
    ...
    top_p_tensor = torch.full((batch_size,), p, device=get_device(0))
    top_k_tensor = torch.full((batch_size,), k, device=get_device(0))
...
def test_top_p_renorm_probs(batch_size, vocab_size, p):
    ...
    sorted_prob, indices = torch.sort(normalized_prob, descending=False)
    cdf = torch.cumsum(sorted_prob, dim=-1)
    mask = torch.zeros(batch_size, vocab_size, dtype=torch.int32, device=get_device(0))
...
def test_min_p_sampling(batch_size, vocab_size, p):
    ...
    min_p_tensor = torch.full((batch_size,), p, device=get_device(0))
