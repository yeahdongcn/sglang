...
def test_static_contract_rejects_mismatched_qk_rms_epsilons():
    spec = QWEN3_06B_METAL_SPEC
    ...
    module = SimpleNamespace(
        ...
        rotary_emb=SimpleNamespace(
            cos_sin_cache=torch.empty(
                8, spec.head_dim, device="mps", dtype=torch.bfloat16
            ),
            is_neox_style=True,
    ...
    with pytest.raises(RuntimeError, match="matching finite positive"):
