...
class TestMLXAttentionWrapper(CustomTestCase):
    ...
    def _make_paged_wrapper(
        ...
        class Inner(nn.Module):
            ...
            def rope(self, x, offset):
                self.rope_offsets.append(offset)
                return x
        ...
        class KVPool:
            if block_size == 1:
                ...
                k_buffer = [
                    mx.zeros((16, block_size, n_kv_heads, head_dim), dtype=cache_dtype)
                ]
                v_buffer = [
                    mx.zeros((16, block_size, n_kv_heads, head_dim), dtype=cache_dtype)
                ]

            def set_kv(self, layer_idx, slot_mapping, keys, values):
                if block_size == 1:
                    self.k_buffer[layer_idx][slot_mapping] = keys
                    self.v_buffer[layer_idx][slot_mapping] = values
                else:
                    block_ids = slot_mapping // block_size
                    block_offsets = slot_mapping % block_size
                    self.k_buffer[layer_idx][block_ids, block_offsets] = keys
                    self.v_buffer[layer_idx][block_ids, block_offsets] = values

        if is_prefill:
            ctx = PagedAttentionContext(
                is_prefill=True,
                slot_mapping=list(range(1, B * L + 1)),
                block_tables=[list(range(L + radix_prefix_len)) for _ in range(B)],
                context_lens=[L + radix_prefix_len] * B,
                offsets=[radix_prefix_len] * B,
                cu_seqlens=[i * L for i in range(B + 1)],
                max_seqlen_q=L,
                max_seqlen_k=L + radix_prefix_len,
                radix_prefix_lens=[radix_prefix_len] * B,
                kv_pool=KVPool(),
            )
        else:
            context_lens = [2, 3][:B]
    ...
    def test_paged_decode_path_uses_mlx_fast_over_paged_cache(self):
        ...
        with (
            patch.dict("sys.modules", {"sgl_kernel.metal": metal_module}),
            patch.object(
                mx.fast, "scaled_dot_product_attention", return_value=sentinel
            ) as dense_attention,
        ):
        ...
        self.assertEqual(k_cache_arg.shape, (16, 1, 4))
        self.assertEqual(v_cache_arg.shape, (16, 1, 4))
        ...
        query_arg, k_arg, v_arg = dense_attention.call_args.args[:3]
        self.assertEqual(query_arg.shape, (2, 2, 1, 4))
        self.assertEqual(k_arg.shape, (2, 1, 3, 4))
        self.assertEqual(v_arg.shape, (2, 1, 3, 4))
        self.assertEqual(dense_attention.call_args.kwargs["scale"], 0.5)
        self.assertEqual(dense_attention.call_args.kwargs["mask"].shape, (2, 1, 1, 3))

    def test_paged_decode_h128_b16_uses_metal_paged_kernel(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
            head_dim=128,
            block_size=16,
        )
        sentinel = mx.ones((2, 2, 1, 128), dtype=mx.float16) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_lazy_unchecked = MagicMock(
            return_value=sentinel
        )

        with (
            patch.dict("sys.modules", {"sgl_kernel.metal": metal_module}),
            patch.object(mx.fast, "scaled_dot_product_attention") as dense_attention,
        ):
            out = wrapper._paged_attention(x, ctx)
        ...
        self.assertTrue(mx.allclose(out, expected))
        metal_module.paged_kv_scatter.assert_called_once()
        metal_module.decode_attention_paged_lazy_unchecked.assert_called_once()
        (
            query_arg,
            k_cache_arg,
            v_cache_arg,
            ...
            context_lens_arg,
            scale_arg,
        ) = metal_module.decode_attention_paged_lazy_unchecked.call_args.args
        self.assertEqual(query_arg.shape, (2, 2, 1, 128))
        self.assertEqual(k_cache_arg.shape, (16, 16, 1, 128))
        self.assertEqual(v_cache_arg.shape, (16, 16, 1, 128))
        self.assertEqual(block_tables_arg.shape, (2, 3))
        self.assertEqual(context_lens_arg.shape, (2,))
        self.assertEqual(scale_arg, 0.5)
        dense_attention.assert_not_called()

    def test_paged_decode_h128_b16_tiny_single_batch_uses_mlx_fast(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=1,
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
        ...
        sentinel = mx.ones((1, 2, 1, 128), dtype=mx.float16) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_lazy_unchecked = MagicMock()
        ...
        expected = sentinel.transpose(0, 2, 1, 3).reshape(1, 1, -1)
        self.assertTrue(mx.allclose(out, expected))
        metal_module.decode_attention_paged_lazy_unchecked.assert_not_called()
        dense_attention.assert_called_once()

    def test_paged_decode_h128_b16_short_single_batch_uses_metal_paged_kernel(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=1,
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
        ...
        ctx.block_tables = mx.array([[0, 1]], dtype=mx.int32)
        ctx.context_lens = mx.array([17], dtype=mx.int32)
        ctx.context_len_values = [17]
        sentinel = mx.ones((1, 2, 1, 128), dtype=mx.float16) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_lazy_unchecked = MagicMock(
    ...
    def test_paged_decode_p1_h128_batched_uses_metal_paged_kernel(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
            head_dim=128,
        ...
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_p1_lazy_unchecked = MagicMock(
            return_value=sentinel
        )
        ...
        self.assertTrue(mx.allclose(out, expected))
        metal_module.paged_kv_scatter.assert_called_once()
        metal_module.decode_attention_paged_p1_lazy_unchecked.assert_called_once()
        (
            query_arg,
            k_cache_arg,
            v_cache_arg,
            ...
            context_lens_arg,
            scale_arg,
        ) = metal_module.decode_attention_paged_p1_lazy_unchecked.call_args.args
        self.assertEqual(query_arg.shape, (2, 2, 1, 128))
        self.assertEqual(k_cache_arg.shape, (16, 1, 128))
        self.assertEqual(v_cache_arg.shape, (16, 1, 128))
        self.assertEqual(block_tables_arg.shape, (2, 3))
        self.assertEqual(context_lens_arg.shape, (2,))
        self.assertEqual(scale_arg, 0.5)
        dense_attention.assert_not_called()

    def test_paged_decode_p1_bf16_h128_batched_uses_metal_paged_kernel(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            projection_dtype=mx.bfloat16,
            cache_dtype=mx.bfloat16,
            head_dim=128,
            block_size=1,
        )
        sentinel = mx.ones((2, 2, 1, 128), dtype=mx.bfloat16) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_p1_lazy_unchecked = MagicMock(
            return_value=sentinel
        ...
        expected = sentinel.transpose(0, 2, 1, 3).reshape(2, 1, -1)
        self.assertTrue(mx.allclose(out, expected))
        metal_module.paged_kv_scatter.assert_not_called()
        metal_module.decode_attention_paged_p1_lazy_unchecked.assert_called_once()
        dense_attention.assert_not_called()

    def test_paged_decode_p1_h128_tiny_single_batch_uses_mlx_fast(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=1,
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
        ...
        ctx.block_tables = mx.array([list(range(16))], dtype=mx.int32)
        ctx.context_lens = mx.array([16], dtype=mx.int32)
        ctx.context_len_values = [16]
        sentinel = mx.ones((1, 2, 1, 128), dtype=mx.float16) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_p1_lazy_unchecked = MagicMock()
        ...
        expected = sentinel.transpose(0, 2, 1, 3).reshape(1, 1, -1)
        self.assertTrue(mx.allclose(out, expected))
        metal_module.decode_attention_paged_p1_lazy_unchecked.assert_not_called()
        dense_attention.assert_called_once()

    def test_paged_decode_p1_h128_short_single_batch_uses_metal_paged_kernel(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=1,
            projection_dtype=mx.float16,
            cache_dtype=mx.float16,
        ...
        ctx.block_tables = mx.array([list(range(17))], dtype=mx.int32)
        ctx.context_lens = mx.array([17], dtype=mx.int32)
        ctx.context_len_values = [17]
        sentinel = mx.ones((1, 2, 1, 128), dtype=mx.float16) * 12
        metal_module = MagicMock()
    ...
    def test_no_prefix_prefill_uses_dense_attention_and_scatters_kv(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=1, seq_len=3, is_prefill=True, radix_prefix_len=0
        )
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.prefill_attention_paged = MagicMock()
        sentinel = mx.ones((1, 2, 3, 4), dtype=mx.float32) * 11
        ...
        with (
            ...
            out = wrapper._paged_attention(x, ctx, mask=None, cache="cache")

        expected = sentinel.transpose(0, 2, 1, 3).reshape(1, 3, -1)
        self.assertTrue(mx.allclose(out, expected))
        metal_module.paged_kv_scatter.assert_called_once()
        metal_module.prefill_attention_paged.assert_not_called()
        dense_attention.assert_called_once()
        q_arg, k_arg, v_arg = dense_attention.call_args.args[:3]
        self.assertEqual(q_arg.shape, (1, 2, 3, 4))
        self.assertEqual(k_arg.shape, (1, 1, 3, 4))
        self.assertEqual(v_arg.shape, (1, 1, 3, 4))
        self.assertEqual(dense_attention.call_args.kwargs["cache"], "cache")
        self.assertEqual(dense_attention.call_args.kwargs["scale"], 0.5)
        self.assertEqual(dense_attention.call_args.kwargs["mask"], "causal")
        self.assertEqual(ctx.kv_scatter_layer_ids, {0})
    ...
    def test_paged_attention_casts_projection_tensors_to_cache_dtype(self):
        ...
        query_arg = dense_attention.call_args.args[0]
        self.assertEqual(query_arg.dtype, mx.float16)

    def test_paged_attention_passes_scalar_rope_offset_for_single_batch(self):
        wrapper, ctx, x = self._make_paged_wrapper(batch_size=1)
        sentinel = mx.ones((1, 2, 1, 4), dtype=mx.float32) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.decode_attention_paged_unchecked = MagicMock(return_value=sentinel)

        with patch.dict("sys.modules", {"sgl_kernel.metal": metal_module}):
            wrapper._paged_attention(x, ctx)

        self.assertEqual(wrapper._inner.rope_offsets, [1, 1])
        self.assertTrue(
            all(isinstance(offset, int) for offset in wrapper._inner.rope_offsets)
        )

    def test_paged_attention_reuses_scalar_rope_offset_for_uniform_batch(self):
        wrapper, ctx, x = self._make_paged_wrapper(
            batch_size=2, seq_len=1, is_prefill=True
        )
        sentinel = mx.ones((2, 2, 4), dtype=mx.float32) * 14
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        metal_module.prefill_attention_paged = MagicMock(return_value=sentinel)
    ...
    def test_paged_attention_applies_per_row_rope_offsets_for_mixed_batch(self):
        wrapper, ctx, x = self._make_paged_wrapper()
        sentinel = mx.ones((2, 2, 1, 4), dtype=mx.float32) * 12
        metal_module = MagicMock()
        metal_module.paged_kv_scatter = MagicMock()
        ...
        self.assertEqual(wrapper._inner.rope_offsets, [1, 2, 1, 2])
        self.assertTrue(
            all(isinstance(offset, int) for offset in wrapper._inner.rope_offsets)
        )
    ...
    def test_paged_attention_does_not_mark_scatter_before_scatter_succeeds(self):
        ...
        metal_module.decode_attention_paged_unchecked = MagicMock()
        ...
        metal_module.decode_attention_paged_unchecked.assert_not_called()
    ...
    def test_call_uses_paged_context_without_env_flag(self):
        ...
        paged_attention.assert_called_once_with(x, ctx, mask=None, cache="cache")
