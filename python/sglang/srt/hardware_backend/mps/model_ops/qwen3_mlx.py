"""Whole-transformer MLX islands for Torch-owned Qwen3-0.6B.
...
Eligible decode batches may use the whole-model island.  Prefill uses it only
for a bounded, prefix-free, single-request batch whose final chunk is present.
Any unsupported dynamic request shape stays on the standard Torch/Metal path;
storage replacement after installation is a lifecycle violation and fails
loudly.  There is no per-layer framework crossing and no post-commit host
fence.
"""
...
QWEN3_COLD_PREFILL_MIN_TOKENS = 512
QWEN3_COLD_PREFILL_MAX_TOKENS = 1024
...
def _single_cpu_int(value: Any) -> Optional[int]:
    """Return one host scalar without synchronizing an accelerator tensor."""
...
class _CompiledBs1Decode:
    ...
    compiled_greedy: Optional[Callable[..., Any]] = None
    warmup_count: int = 0
    call_count: int = 0
    fallback_count: int = 0
    greedy_warmup_count: int = 0
    greedy_call_count: int = 0

    @classmethod
    def create(
        cls, views: _DecodeViews, *, compile_greedy_tail: bool = False
    ) -> _CompiledBs1Decode:
        import mlx.core as mx
        ...
        def greedy_graph(
            input_ids, positions, req_to_token, req_pool_indices, seq_lens
        ):
            graph_views = _unflatten_decode_arrays(bundle.captures, bundle.static)
            return _mlx_decode_greedy_graph(
                graph_views,
                input_ids,
                positions,
                req_to_token,
        ...
        # A compiled Qwen3 graph contains all 28 decoder layers.  Keep exactly
        # one primary variant resident on memory-constrained Apple Silicon:
        # the hidden-output graph for the Torch tail, or the token-output graph
        # for the MLX greedy tail.  Requests that need the non-primary output
        # remain correct through the eager graph instead of retaining a second
        # full compiled executable.
        if compile_greedy_tail:
            bundle.compiled_greedy = mx.compile(
                greedy_graph, inputs=captures, shapeless=False
            )
        else:
    ...
    def can_run_greedy(self, inputs: tuple[Any, ...]) -> bool:
        return self.compiled_greedy is not None and self.can_run(inputs)
    ...
    def greedy(self, *inputs: Any):
        signature = _array_signature(inputs)
        if self.dynamic_signature is None:
            self.dynamic_signature = signature
        elif signature != self.dynamic_signature:
            raise RuntimeError("Qwen3 compiled MLX greedy input signature changed")
        if self.compiled_greedy is None:
            raise RuntimeError("Qwen3 compiled MLX greedy graph is not enabled")
        self.greedy_call_count += 1
        return self.compiled_greedy(*inputs)
    ...
    def warmup_greedy(self, *inputs: Any):
        """Compile/evaluate the greedy signature without counting a request."""
        signature = _array_signature(inputs)
        if self.dynamic_signature is None:
            self.dynamic_signature = signature
        elif signature != self.dynamic_signature:
            raise RuntimeError("Qwen3 compiled MLX greedy warmup signature changed")
        if self.compiled_greedy is None:
            raise RuntimeError("Qwen3 compiled MLX greedy graph is not enabled")
        self.greedy_warmup_count += 1
        return self.compiled_greedy(*inputs)
...
_GREEDY_TAIL_DISABLED_REASON = "disabled by SGLANG_MPS_QWEN3_GREEDY_TAIL priority"


def _greedy_tail_static_fallback_reason(
    model: Any,
    server_args: Any,
    *,
    enabled: bool,
) -> Optional[str]:
    """Validate static equivalence with Qwen3's ordinary Torch greedy tail."""
    if not enabled:
        return _GREEDY_TAIL_DISABLED_REASON
    sampling_backend = getattr(server_args, "sampling_backend", None)
    sampling_backend = getattr(sampling_backend, "value", sampling_backend)
    if server_args is None or sampling_backend != "pytorch":
        return "sampling backend is not pytorch"
    if bool(getattr(server_args, "enable_fp32_lm_head", False)):
        return "FP32 LM head is enabled"
    if bool(getattr(server_args, "enable_dp_lm_head", False)):
        return "DP LM head is enabled"
    config = getattr(model, "config", None)
    qwen3_model = getattr(model, "model", None)
    embedding = getattr(qwen3_model, "embed_tokens", None)
    lm_head = getattr(model, "lm_head", None)
    if not bool(getattr(config, "tie_word_embeddings", False)):
        return "LM head is not tied to the token embedding"
    if lm_head is not embedding:
        return "LM head and token embedding are different modules"
    weight = getattr(embedding, "weight", None)
    vocab_size = int(getattr(config, "vocab_size", -1))
    if not isinstance(weight, torch.nn.Parameter) or tuple(weight.shape) != (
        vocab_size,
        QWEN3_06B_HIDDEN_SIZE,
    ):
        return "tied embedding does not match the logical vocabulary"
    logits_processor = getattr(model, "logits_processor", None)
    if logits_processor is None:
        return "logits processor is unavailable"
    if getattr(logits_processor, "logit_scale", None) is not None:
        return "logit scaling is enabled"
    if getattr(logits_processor, "final_logit_softcapping", None):
        return "final logit softcapping is enabled"
    if bool(getattr(logits_processor, "use_fp32_lm_head", False)):
        return "logits processor uses an FP32 LM head"
    if bool(getattr(logits_processor, "do_tensor_parallel_all_gather", False)):
        return "LM-head tensor-parallel gather is enabled"
    if bool(getattr(logits_processor, "do_tensor_parallel_all_gather_dp_attn", False)):
        return "LM-head DP gather is enabled"
    if bool(getattr(logits_processor, "return_full_logits", False)):
        return "full logits are requested"
    return None
...
def _swiglu(gate, up):
    ...
    The function is a deliberately small model-op seam.  The default keeps
    the same algebra as the Torch reference; a benchmark or a future MPS
    provider plan may replace it with MLX's shapeless-compiled ``silu``
    helper without changing model ownership or the surrounding graph.
    """
    import mlx.core as mx
...
def _rope_neox_qk(q, k, cos_sin, positions):
    """Apply the same NeoX rotation to Q and K in one MLX primitive chain.

    Qwen3 uses the same head dimension, RoPE cache, and positions for both
    tensors.  Concatenating only along the head axis lets MLX share the cache
    gather, broadcasts, and elementwise rotation across Q/K.  The final Q/K
    slices are views, so the shared rotation does not require a second rotated
    allocation.
    """
    import mlx.core as mx

    q_heads = q.shape[1]
    rotated = _rope_neox(mx.concatenate((q, k), axis=1), cos_sin, positions)
    return rotated[:, :q_heads, :], rotated[:, q_heads:, :]


def _prepare_qkv(qkv, layer, positions, *, share_qk_rope: bool = False):
    ...
    if share_qk_rope:
        return (*_rope_neox_qk(q, k, layer.rope_cache.array, positions), v)
    return (
...
def _mlx_greedy_token(views: _DecodeViews, hidden, *, last_only: bool):
    """Project the selected hidden rows through the tied embedding and argmax."""
    import mlx.core as mx

    selected = hidden[-1:] if last_only else hidden
    logits = selected @ mx.transpose(views.embedding.array)
    return mx.argmax(logits, axis=-1).astype(mx.int64)


def _mlx_decode_greedy_graph(
    views: _DecodeViews,
    input_ids,
    positions,
    req_to_token,
    ...
    hidden, new_k, new_v = _mlx_decode_graph(
        views,
        input_ids,
        positions,
        req_to_token,
    ...
    return _mlx_greedy_token(views, hidden, last_only=False), new_k, new_v
...
def _mlx_cold_prefill_graph(
    ...
    *,
    ...
    """Build one bounded prefix-free causal graph for a single request."""

    def attention_forward(_layer, q, k, v):
        # The provider admits exactly one complete prefix-free request, so the
        # flattened token dimension is the sequence dimension. MLX handles
        # the native 16:8 grouped-query layout without tiling K/V.
        return _mlx_causal_gqa(q, k, v)

    return _mlx_model_graph(
        views,
        input_ids,
        positions,
        attention_forward,
...
def _mlx_cold_prefill_greedy_graph(
    views: _DecodeViews,
    input_ids,
    positions,
):
    hidden, new_k, new_v = _mlx_cold_prefill_graph(views, input_ids, positions)
    return _mlx_greedy_token(views, hidden, last_only=True), new_k, new_v


@dataclass
class Qwen3MlxModelProvider:
    """Bounded MLX forward provider installed on one typed Qwen3 model."""

    views: Optional[_DecodeViews]
    model_identity: Optional[int] = None
    greedy_tail_backend: str = "torch"
    greedy_tail_static_fallback_reason: Optional[str] = None
    # Deferred K/V commit is a separate semantic operation from the coarse
    # MLX model forward.  Pin it at startup so the provider never hardcodes a
    # Metal implementation or silently changes provider after publication.
    ...
    prefill_call_count: int = 0
    selector_call_count: int = 0
    selector_fallback_count: int = 0
    last_selector_fallback_reason: Optional[str] = None
    greedy_tail_call_count: int = 0
    greedy_tail_torch_call_count: int = 0
    greedy_tail_fallback_count: int = 0
    last_greedy_tail_fallback_reason: Optional[str] = None
    started: bool = False
    closed: bool = False
    ...
    def __post_init__(self) -> None:
        if self.greedy_tail_backend not in {"mlx", "torch"}:
            raise ValueError(
                "Qwen3 MLX greedy tail backend must be 'mlx' or 'torch'; "
                f"found {self.greedy_tail_backend!r}"
            )
        if (
            self.greedy_tail_backend == "torch"
            and self.greedy_tail_static_fallback_reason is None
        ):
            self.greedy_tail_static_fallback_reason = _GREEDY_TAIL_DISABLED_REASON
    ...
    def start(self, req_to_token_pool: Any) -> None:
        """Compile and warm every owned pipeline before provider publication."""
        if self.closed:
    ...
    def enable_compiled_decode(self) -> None:
        """Create the gate-selected bs=1 ``mx.compile`` primary executable."""
        if self.closed:
        ...
        self._compiled_decode = _CompiledBs1Decode.create(
            self.views,
            compile_greedy_tail=self.greedy_tail_static_fallback_reason is None,
        )
    ...
    def warmup_compiled_decode(self, req_to_token: torch.Tensor) -> None:
        ...
        if compiled.compiled is not None:
            ...
            mx.eval(*outputs)
        if compiled.compiled_greedy is not None:
            outputs = compiled.warmup_greedy(*inputs)
            mx.eval(*outputs)
    ...
    def close(self) -> None:
        """Release borrowed graph state at an explicit worker lifecycle boundary."""
        if self.closed:
            return
    ...
    def get_compiled_decode_state(self) -> dict[str, Any]:
        compiled = self._compiled_decode
        hidden_warmups = int(getattr(compiled, "warmup_count", 0))
        hidden_calls = int(getattr(compiled, "call_count", 0))
        greedy_warmups = int(getattr(compiled, "greedy_warmup_count", 0))
        greedy_calls = int(getattr(compiled, "greedy_call_count", 0))
        hidden_enabled = bool(compiled is not None and compiled.compiled is not None)
        greedy_enabled = bool(
            compiled is not None and compiled.compiled_greedy is not None
        )
        return {
            "enabled": hidden_enabled,
            "total_enabled": hidden_enabled or greedy_enabled,
            "primary_variant": (
                "hidden" if hidden_enabled else "greedy" if greedy_enabled else "off"
            ),
            # Preserve the pre-greedy hidden-graph counter semantics.  Explicit
            # totals describe whichever single primary graph the gate selected.
            "warmup_count": hidden_warmups,
            "call_count": hidden_calls,
            "total_warmup_count": hidden_warmups + greedy_warmups,
            "total_call_count": hidden_calls + greedy_calls,
            "fallback_count": int(getattr(compiled, "fallback_count", 0)),
            "greedy_enabled": greedy_enabled,
            "greedy_warmup_count": greedy_warmups,
            "greedy_call_count": greedy_calls,
        }

    def _greedy_tail_fallback_reason(self, forward_batch: Any) -> Optional[str]:
        return self.greedy_tail_static_fallback_reason or (
            precomputed_greedy_fallback_reason(forward_batch)
        )
    ...
    def _cold_prefill_fallback_reason(
        self,
        forward_batch: Any,
        *,
        model: Any,
        ...
        if getattr(forward_batch, "forward_mode", None) is not ForwardMode.EXTEND:
            return "forward_mode"
        reason = self._common_fallback_reason(
            forward_batch,
            model=model,
        ...
        if not (
            ...
            return "prefill_token_range"
        if int(getattr(forward_batch, "batch_size", -1)) != 1:
            return "prefill_batch_size"
        if not bool(getattr(forward_batch, "contains_last_prefill_chunk", False)):
            return "incomplete_prefill_chunk"
        if int(getattr(forward_batch, "extend_num_tokens", -1)) != token_count:
            return "extend_num_tokens"
        if (
            _single_cpu_int(getattr(forward_batch, "extend_seq_lens_cpu", None))
            != token_count
        ):
            return "extend_seq_lens"
        if _single_cpu_int(getattr(forward_batch, "extend_prefix_lens_cpu", None)) != 0:
            return "prefix_hit"
        if _single_cpu_int(getattr(forward_batch, "seq_lens_cpu", None)) != token_count:
            return "seq_lens"
        if (
            int(getattr(forward_batch, "num_token_non_padded_cpu", -1) or -1)
            != token_count
        ):
            return "padded_prefill"
        if not _is_mps_vector(
            getattr(forward_batch, "out_cache_loc", None),
            dtype=torch.int64,
            length=token_count,
        ):
            return "out_cache_loc_layout"
        return None
    ...
    def should_run(
        ...
        *,
        ...
        if mode is ForwardMode.DECODE:
            ...
            reason = self._cold_prefill_fallback_reason(
                forward_batch,
                model=model,
                input_ids=input_ids,
                positions=positions,
    ...
    def invalidate_views(self) -> None:
        if self.closed:
            return
        self.views = None
        # Do not release the last DLPack commit sources here.  The next bridge
        # fence proves the Torch Metal commit has consumed them; until then the
        # provider keeps their MLX-owned storage alive.
    ...
    def _after_mps_fence(self, views: _DecodeViews) -> None:
        """Publish refreshed captures only after all older Torch reads finish."""
    ...
    def _validate_decode_inputs(
        ...
        return kv_pool, req_pool, req_to_token

    def _validate_cold_prefill_inputs(
        self,
        model: Any,
        input_ids: torch.Tensor,
        positions: torch.Tensor,
        ...
        if reason is not None:
            raise RuntimeError(
                "Qwen3 MLX cold-prefill eligibility changed after selection: " + reason
            )
        token_count = int(input_ids.numel())
        for name, tensor in (
            ("input_ids", input_ids),
            ...
            if (
                ...
                or tuple(tensor.shape) != (token_count,)
                or not tensor.is_contiguous()
            ):
                raise RuntimeError(
                    "Qwen3 MLX cold prefill requires contiguous MPS int64 "
                    f"{name}[{token_count}]"
                )
    ...
    def _validate_pool_storage(kv_pool: Any, views: _DecodeViews) -> None:
        if id(kv_pool) != views.pool_identity:
            raise RuntimeError(
                "Qwen3 MLX forward detected a replaced KV pool; reinstall the "
                "runtime operator plan before serving"
            )
        for layer_id, layer in enumerate(views.layers):
            k_pool, v_pool = kv_pool.get_kv_buffer(layer_id)
            if not layer.k_pool.matches(k_pool) or not layer.v_pool.matches(v_pool):
                raise RuntimeError(
                    "Qwen3 MLX forward detected replaced KV storage at layer "
                    f"{layer_id}; reinstall the runtime operator plan before serving"
                )
    ...
    def forward(
        ...
        is_cold_prefill = (
            getattr(forward_batch, "forward_mode", None) is ForwardMode.EXTEND
        )
        req_to_token = None
        if is_cold_prefill:
            kv_pool = self._validate_cold_prefill_inputs(
                model,
                input_ids,
                positions,
                forward_batch,
            ...
            kv_pool, _req_pool, req_to_token = self._validate_decode_inputs(
                model,
                input_ids,
                positions,
                forward_batch,
        ...
        greedy_tail_fallback_reason = self._greedy_tail_fallback_reason(forward_batch)
        use_precomputed_greedy = greedy_tail_fallback_reason is None
        self.last_greedy_tail_fallback_reason = greedy_tail_fallback_reason
        if not use_precomputed_greedy:
            self.greedy_tail_torch_call_count += 1
            # A disabled gate deliberately selected the Torch tail; it is not
            # an MLX fallback.  Count only requests for which MLX was selected
            # but a static or request-level correctness condition rejected it.
            if self.greedy_tail_backend == "mlx":
                self.greedy_tail_fallback_count += 1
        views = self.views
        if views is None:
            # The bridge immediately below performs the one producer fence for
            # these refreshed views and all dynamic scheduler tensors.
        ...
        if not is_cold_prefill:
            assert req_to_token is not None
            maybe_detect_oob(
        ...
        # Retire the preceding asynchronous commit immediately after the
        # bridge's existing Torch producer fence.  This releases a cold
        # prefill's potentially large stacked K/V outputs before constructing
        # and evaluating the next MLX graph, without adding another fence.
        compiled = self._compiled_decode
        ...
        if is_cold_prefill:
            graph = (
                _mlx_cold_prefill_greedy_graph
                if use_precomputed_greedy
                else _mlx_cold_prefill_graph
            )
            model_output, new_k, new_v = mlx_call_multi(
                lambda ids, pos: graph(views, ids, pos),
                input_ids,
                positions,
            ...
            if (
                ...
                and use_precomputed_greedy
                and compiled.can_run_greedy(dynamic_inputs)
            ):
                graph = compiled.greedy
            elif (
                compiled is not None
                and not use_precomputed_greedy
                and compiled.can_run_hidden(dynamic_inputs)
            ):
                graph = compiled
            else:
                if compiled is not None:
                ...
                def graph(ids, pos, table, req, lens):
                    graph_fn = (
                        ...
                        if use_precomputed_greedy
                        else _mlx_decode_graph
                    )
                    return graph_fn(views, ids, pos, table, req, lens)
        ...
        # Publish the ownership before either launch.  If validation or the
        # second launch raises, the MLX-exported buffers remain alive and the
        ...
        if use_precomputed_greedy:
            self.greedy_tail_call_count += 1
            return LogitsProcessorOutput(
                next_token_logits=None,
                precomputed_greedy_token_ids=model_output,
            )
        return model_output
...
def create_qwen3_mlx_model_provider(
    ...
    *,
    ...
    """Validate, compile, warm, and return an unpublished coarse provider."""
    from sglang.srt.models.qwen3 import Qwen3ForCausalLM

    if greedy_tail_backend not in {"mlx", "torch"}:
        raise ValueError(
            "Qwen3 MLX greedy tail backend must be 'mlx' or 'torch'; "
            f"found {greedy_tail_backend!r}"
        )
    ...
    provider = Qwen3MlxModelProvider(
        ...
        greedy_tail_backend=greedy_tail_backend,
        greedy_tail_static_fallback_reason=_greedy_tail_static_fallback_reason(
            model,
            server_args,
            enabled=greedy_tail_backend == "mlx",
        ),
