...
def _metal_source(spec: Qwen3MetalKernelSpec) -> str:
    """Read the canonical source shared by the JIT and AOT build paths."""
...
def _compile_qwen3_library(spec: Qwen3MetalKernelSpec):
    ...
    if not callable(compile_shader):
        raise RuntimeError(
            "Qwen3 Metal JIT requires torch.mps.compile_shader from Torch 2.13"
        )
    return compile_shader(_metal_source(spec))
...
def warmup_qwen3_metal_kernels(
    ...
    *,
    ...
    if not qknorm_rope_store and not radix_decode:
        return
    library = _compile_qwen3_library(spec)
