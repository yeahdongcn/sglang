...
implementations are correctness references; the JIT implementations share a
fixed Qwen3-0.6B shader and execute on Torch's MPS stream through
``torch.mps.compile_shader``.

``metal_aot`` remains a separately selectable provider kind, but is reported
unavailable until the Apple kernel wheel ships a Torch data-only metallib.
The legacy MLX/nanobind extension is deliberately not used as that artifact.
"""
...
# Model-plan installation replaces this per semantic operation. Keep direct
# imports and calls on the correctness path until a gate is explicitly read.
_PRIORITY = (KernelBackend.TORCH,)
...
def is_qwen3_metal_aot_available() -> bool:
    """Whether the future Torch data-only Qwen3 metallib is installed."""
    return False
    ...
    *,
    ...
    def warm_one(selected: KernelBackend, *, qknorm: bool) -> None:
        ...
        if selected is KernelBackend.METAL_AOT:
            raise RuntimeError(
                "Qwen3 Metal AOT is unavailable without the Torch data-only "
                "metallib wheel"
            )
