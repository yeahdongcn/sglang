"""Torch-owned Metal JIT kernels for the 1024-wide RMSNorm contract.

The implementation is deliberately narrow: inference-only, contiguous bf16
2-D tensors with hidden size 1024.  Callers must check the exported
``can_use_*`` predicates and retain a native Torch fallback for every other
shape, dtype, layout, and autograd mode.
"""
...
@lru_cache(maxsize=1)
def _rmsnorm_kernel():
    library = compile_metal_library(_METAL_SOURCE)
    return resolve_metal_entry_points(library, ("rmsnorm_1024_bf16",))[0]


@lru_cache(maxsize=1)
def _fused_add_rmsnorm_kernel():
    library = compile_metal_library(_METAL_SOURCE)
    return resolve_metal_entry_points(library, ("fused_add_rmsnorm_1024_bf16",))[0]
...
def can_use_mps_rmsnorm(
    ...
    *,
    ...
    if tuple(weight.shape) != (_HIDDEN_SIZE,) or input.shape[0] == 0:
        return False
    if _shares_storage(input, weight):
        return False
...
def can_use_mps_fused_add_rmsnorm(
    ...
    if torch.is_grad_enabled() and residual.requires_grad:
        return False
    return not (_shares_storage(input, residual) or _shares_storage(residual, weight))
...
def warmup_mps_rmsnorm_kernels(
    *, rmsnorm: bool = True, fused_add_rmsnorm: bool = True
    ...
    """Compile and resolve only the selected pipelines before serving."""
    if rmsnorm:
        _rmsnorm_kernel()
